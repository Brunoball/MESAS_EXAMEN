...
try {
  ...
  if (!$ids) {
    // AUN ASÍ limpiamos SIEMPRE mesas_no_agrupadas
    $deleted_no_agrupadas = 0;
    $deleted_no_agrupadas_incons = 0;

    try {
      // contamos antes de borrar (por si querés ver cuántos había)
      $deleted_no_agrupadas = (int)($pdo->query("SELECT COUNT(*) FROM mesas_no_agrupadas")->fetchColumn() ?: 0);
  ...
  // 3.b) LIMPIEZA COMPLETA de mesas_no_agrupadas (SIEMPRE)
  $deleted_no_agrupadas = 0;
  try {
    // contamos cuántos había antes
    $deleted_no_agrupadas = (int)($pdo->query("SELECT COUNT(*) FROM mesas_no_agrupadas")->fetchColumn() ?: 0);
    // limpiamos todo
    $pdo->exec("TRUNCATE TABLE mesas_no_agrupadas");
  } catch (Throwable $e) {
  ...
  // 4) limpieza extra de seguridad (inconsistencias en grupos)
  ...
  // 4.b) ya NO necesitamos limpieza extra de mesas_no_agrupadas por inconsistencias
  $deleted_no_agrupadas_incons = 0;
  ...
  if ($restantes === 0) {
    try {
      ...
      // mesas_no_agrupadas ya se truncó antes, no hace falta repetir
      $reiniciado = true;
