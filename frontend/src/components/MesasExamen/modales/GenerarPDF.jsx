 ...
 * - Cada mesa imprime un subtítulo y su tabla; pueden cortarse entre páginas.
 * - Evita que el encabezado de tabla quede huérfano al final de la hoja.
 * - El logo preserva su aspecto (sin “achatado”).
 */
export async function generarPDFMesas({
  ...
  try {
    ...
    const MIN_ROWS_AFTER_HEAD = 2;  // queremos al menos 2 filas tras el head

    // Logo (con aspecto preservado)
    let logoImg = null;
    ...
    try {
      ...
      logoW = sz.w; logoH = sz.h;
    } catch {/* sin logo, no pasa nada */}

    // Altura del header de página
    const HEADER_H = Math.max(logoH, 44) + 18; // título centrado + respiro
    const CONTENT_TOP = MTOP + HEADER_H + HEADER_GAP;

    // Anchos de columnas (se escalan al ancho útil)
    ...
    // Tipografías
    const FT_TITLE = 16, FT_SUB = 10, FT_HEAD = 9, FT_BODY = 9, PAD = 4;

    // Header de página estático (dibujar SIEMPRE al entrar a una página)
    const drawPageHeader = () => {
      if (logoImg) {
        doc.addImage(logoImg, "PNG", ML, MTOP, logoW, logoH);
      }
      doc.setFont("helvetica", "bold"); doc.setFontSize(FT_TITLE);
      // centramos verticalmente el título sobre el bloque del logo
      const titleY = MTOP + Math.max(18, Math.ceil(logoH * 0.5));
      doc.text("MESAS DE EXAMEN", pageW / 2, titleY, { align: "center" });
    ...
    // Subtítulo por mesa (fecha/turno/hora + N° de mesa)
    const drawMesaCaption = (mesa, y) => {
      ...
      const turnoTxt = `${String(mesa.turno || "").toUpperCase()} · ${HORA_POR_TURNO(mesa.turno)}`;
      ...
      // Línea fina debajo del caption
      doc.setDrawColor(200); doc.setLineWidth(0.6);
      doc.line(ML, y + 16, pageW - ML, y + 16);

      return y + 20; // próxima Y para comenzar la tabla
    };

    // Cuerpo de la tabla para una mesa
    const buildBody = (mesa) => {
      ...
      const segDocente = [];
      let curDoc = null, accDoc = 0, startDoc = 0;
      let rowCursor2 = 0;
      for (let i = 0; i < mesa.bloques.length; i++) {
      ...
      for (let idx = 0; idx < mesa.bloques.length; idx++) {
        ...
        for (let i = 0; i < n; i++) {
          ...
          // Alumno / DNI / Curso
          row.push(String(a.alumno || ""));     // Estudiante
          row.push(String(a.dni || ""));        // DNI
          row.push(limpiarCurso(a.curso));      // Curso
    ...
    // ====== Render apilado ======
    // Dibujo header de la PRIMERA página
    drawPageHeader();

    let currentY = CONTENT_TOP;

    // Estimaciones para evitar “head huérfano”
    const CAPTION_HEIGHT = 20; // lo que ocupa drawMesaCaption (devuelve y + 20)
    const HEAD_APPROX = 18;    // alto aproximado del head de la tabla

    // Recorremos mesas
    for (let idxMesa = 0; idxMesa < mesasLogicas.length; idxMesa++) {
      const mesa = mesasLogicas[idxMesa];

      // Si no hay espacio para caption + head + 2 filas mínimas, salto de página
      const MIN_TABLE_BLOCK = CAPTION_HEIGHT + HEAD_APPROX + (MIN_ROWS_AFTER_HEAD * 12); // 12 ≈ altura de fila
      ...
      // Render tabla (autoTable la partirá si hace falta)
      autoTable(doc, {
        startY: currentY,
        margin: { top: CONTENT_TOP, bottom: BOTTOM_SAFE, left: ML, right: ML }, // top fijo para NUEVAS páginas
        styles: {
          font: "helvetica",
          fontSize: FT_BODY,
          cellPadding: PAD,
        ...
        tableLineWidth: 0.8,
        theme: "grid",
        pageBreak: "auto",       // permite cortar la tabla entre páginas
        rowPageBreak: "auto",
        ...
        // Redibujo el encabezado de página cada vez que autoTable abre una nueva hoja
        didDrawPage: () => drawPageHeader(),
      });

      // Actualizamos cursor según la última tabla
