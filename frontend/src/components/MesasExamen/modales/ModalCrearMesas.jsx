 ...
 * 3) mesas_reoptimizar
 * 4) mesas_reoptimizar_completar (intenta sumar las mesas sueltas a grupos ya creados)
 */
...
const ModalCrearMesas = ({ open, onClose, onSuccess, onError, onLoadingChange }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3) Reoptimizar (algoritmo principal)
      ...
      if (!respReopt.ok || !jsonReopt?.exito) {
        ...
        onError?.(msg);
        // igual seguimos al paso 4, porque puede terminar de acomodar alguna mesa suelta
      }

      // 4) Reoptimización extra: intentar meter mesas sueltas en grupos ya creados
      //    No le mando fecha/turno: el PHP se encarga de revisar todas las mesas sueltas.
      const { resp: respReoptExtra, json: jsonReoptExtra } = await postJson(
        "mesas_reoptimizar_completar",
        {
          dry_run: 0,
          // si querés limitar por día/turno más adelante:
          // fecha_mesa: fechaInicio,
          // id_turno: 1,
        }
      );

      if (!respReoptExtra.ok || !jsonReoptExtra?.exito) {
        const msg =
          jsonReoptExtra?.mensaje ||
          `La reoptimización extra no se pudo completar [HTTP ${respReoptExtra.status}]`;
        // No lo tomo como error fatal: las mesas ya están creadas y agendadas igual
        onError?.(
          msg +
            " — Igual las mesas se crearon y se agruparon; solo puede que queden algunas sin poder sumarse a otros grupos."
        );
      }
  ...
  // ⏱ Handlers SIN bloqueo de sábados y domingos
